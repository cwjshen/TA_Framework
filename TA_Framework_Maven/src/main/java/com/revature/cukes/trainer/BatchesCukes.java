...
public class BatchesCukes {
	...
	@Given("^I click the focus button batches <webdriver>$")
	public static void i_click_the_focus_button_webdriver(WebDriver wd) throws Throwable {
		BatchesTab.focusFilter(wd).click();
	}

	@Then("^The focus should be filtered in ASC order batches <webdriver>$")
	public static boolean the_focus_should_be_filtered_in_ASC_order_webdriver(WebDriver wd) throws Throwable {
		// Get the list of all batch names
		boolean isSorted = true;
		List<String> curr = new ArrayList<String>();
		for (WebElement col : BatchesTab.getFocus(wd)) {
			curr.add(col.getText());
		}
		curr.removeAll(Arrays.asList("", null));
	...
	@Then("^If I click the focus button again batches <webdriver>$")
	public static void if_I_click_the_focus_button_again_webdriver(WebDriver wd) throws Throwable {
		BatchesTab.focusFilter(wd).click();
	}

	@Then("^The focus should be filtered in DSC order batches <webdriver>$")
	public static boolean the_focus_should_be_filtered_in_DSC_order_webdriver(WebDriver wd) throws Throwable {
		// Get the list of all batch names
		boolean isSorted = true;
		List<String> curr = new ArrayList<String>();
		for (WebElement col : BatchesTab.getFocus(wd)) {
